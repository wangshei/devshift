          ...
          {projects.map(p => {
            ...
            return (
              <div key={p.project.id} className={`group flex items-center gap-2 px-2 py-1.5 rounded-md cursor-pointer transition-colors ${isActive ? 'bg-accent/10' : 'hover:bg-hover'}`}>
                ...
                <button
                  onClick={e => { e.stopPropagation(); navigate(`/project/${p.project.id}/map`); }}
                  title="Product Map"
                  className="hidden group-hover:block text-vmuted hover:text-accent text-xs transition-colors ml-0.5"
                >
